...
└── scripts/                  # All scripts live here (auto-chmod enabled)
...
```bash
wget -qO- https://raw.githubusercontent.com/cbalingbing/ePADDY/main/setup_script_rpi0/bootstrap.sh | bash
```
...
```bash
source ~/Desktop/Epaddy/scripts/setup_part2.sh
```
...
```bash
mkdir -p ~/Desktop/Epaddy/scripts
cd ~/Desktop/Epaddy/scripts
```
...
| Part 2 did not auto-run after reboot | Run manually: `source ~/Desktop/Epaddy/scripts/setup_part2.sh` |
