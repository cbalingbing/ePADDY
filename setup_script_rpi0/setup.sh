...
sudo apt install -y inotify-tools || { echo "ERROR: Failed to install inotify-tools."; echo "Fix: Try: sudo apt install -y inotify-tools"; exit 1; }
...
sudo apt install -y cifs-utils samba smbclient || { echo "ERROR: Failed to install cifs-utils, samba, or smbclient."; echo "Fix: Try: sudo apt install -y cifs-utils samba smbclient"; exit 1; }
...
if [ ! -d "$VENV_PATH" ]; then
    python3 -m venv "$VENV_PATH" --system-site-packages || { echo "ERROR: Failed to create virtual environment."; echo "Fix: Try running: sudo apt install -y python3-venv"; exit 1; }
