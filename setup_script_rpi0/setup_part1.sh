...
PART2_SCRIPT="$TARGET_DIR/setup_part2.sh"
...
if [ ! -d "$VENV_PATH" ]; then
    python3 -m venv "$VENV_PATH" || { echo "ERROR: Failed to create venv."; echo "Fix: sudo apt install -y python3-venv"; exit 1; }
...
pip install -r "$TARGET_DIR/requirements.txt" || { echo "ERROR: pip install failed."; echo "Fix: Check requirements.txt exists in $TARGET_DIR."; exit 1; }
...
sudo "$VENV_PATH/bin/python3" raspi-blinka.py || { echo "ERROR: raspi-blinka.py failed."; echo "Fix: Check I2C and SPI are enabled and your Pi is supported."; exit 1; }
