...
# ========================
interpreter = tf.lite.Interpreter(model_path=r"C:\Users\HP\Desktop\EPaddy Updated\my_model.tflite")
...
def classify(folder_path):
    ...
    if not audio_files:
        return "No .wav files found in the folder."

    # Get the latest file based on modification time
    latest_file = max(audio_files, key=lambda f: os.path.getmtime(os.path.join(folder_path, f)))
    file_path = os.path.join(folder_path, latest_file)
    print(file_path)
    burst_peaks = Burst.detect_burst_peaks(file_path, threshold=0.03, peak_distance=30)

    with tempfile.TemporaryDirectory() as temp_dir:

        processed_filepath, segment_files = preprocess_audio(file_path, output_folder=temp_dir)

        print(f"\nProcessing latest file: {latest_file}")

        # Counter for each species (per file)
        class_counts = {cls: 0 for cls in subdirectories}
        ...
        for seg_file in segment_files:
            ...
            # Increase counter
            class_counts[predicted_species] += 1

            print(f"  Segment {os.path.basename(seg_file)} → {predicted_species}")

        # ===== After all segments, decide final species =====
        final_species = max(class_counts, key=class_counts.get)   # highest class name
        final_count = class_counts[final_species]                 # its count
        ...
        text_message =  f"Warning: multiple detections of insect infestation. The most evident among them is {highest_count['species']} with a total of {highest_count['count']} detections."
...
detect = classify(r"C:\Users\HP\Desktop\EPaddy Updated\retrieve_audio") #Change its folder path
