...
# Retrieve data from Raspberry Pi 4 Incomplete
@app.route('/retrieve_data', methods=['POST'])
def retrieve_data():
    # Define the directory where WAV files are stored
    wav_directory = 'path/to/wav/files/directory'
    
    # Get all subdirectories
    subdirectories = [os.path.join(wav_directory, d) for d in os.listdir(wav_directory) 
                      if os.path.isdir(os.path.join(wav_directory, d))]
    
    # Find the most recent subdirectory
    most_recent_directory = max(subdirectories, key=os.path.getmtime)
    
    # Get all WAV files in the most recent directory
    wav_files = [f for f in os.listdir(most_recent_directory) if f.endswith('.wav')]
    
    # Process and predict for each WAV file
    results = []
    for wav_file in wav_files:
        file_path = os.path.join(most_recent_directory, wav_file)
